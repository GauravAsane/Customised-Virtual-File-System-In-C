...
UFDT UFDTArr[5];	// Array object of structure name as struct UDFT;
...
void man(char *name)
{
	...
	else if(strcmp(name,"write") == 0)
	{
		...
		printf("Usage : write File_name\n After this the data that we want to write\n");		
	}
	...
	{
		printf("Description : used to  file\n");
		printf("Usage : create File_name permission\n");		
	}
...
int main()
{
	...
	while(1)
	{
		...
		//  fgets(kashat , kiti ,kunakdun)
			fgets(str,80,stdin);  // scanf("%d[^'\n']s")	// because of this command which is written ,accept in str array

			count = sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);	// tokenizing
			...
			else if(count == 3)
			{
				...
				else if(strcmp(command[0],"read") == 0)
				{
					...
					if(ret == -4)
					{
						printf("ERROR : In is not regular file\n");
					}
					if(ret == 0)
					{
